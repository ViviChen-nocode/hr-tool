...
export const FloatingMascot: React.FC<FloatingMascotProps> = ({
  ...
  const tailPositionClasses =
    position === 'bottom-right' ? 'right-10' : 'left-10';

  return (
    <div
      className={`${positionClasses} z-50 pointer-events-auto select-none`}
    ...
    >
      {/* Speech bubble */}
      <div
        className={`absolute bottom-full ${bubblePositionClasses} mb-2 transition-all duration-300 ${
          isHovered
      ...
      >
        <div className="bg-card rounded-2xl px-4 py-2 shadow-elevated border border-border/50 whitespace-nowrap flex items-center gap-2">
          ...
          <p className="text-sm font-medium text-foreground">{hoverText}</p>
        </div>
        {/* Bubble tail */}
        <div className={`absolute bottom-0 ${tailPositionClasses} translate-y-full`}>
          <div
            className="w-0 h-0 border-l-8 border-r-8 border-t-8 border-l-transparent border-r-transparent border-t-card"
